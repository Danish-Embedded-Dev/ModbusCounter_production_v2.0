...
//Add Analog Output registers 40001-40020 to the register bank
  ...
  regBank.add(40013);   // for save the configuration to internal flash  
  ...
  regBank.set(40013,  NORMAL_STATE);                       // for save the configuration to internal flash    
}
...
void update_counter_reg(void) {
  ...
  regBank.set(10001, button1.isPressed());    //show input switch 1 current state 
  regBank.set(10002, button2.isPressed());    //show input switch 2 current state
  ...
  regBank.set(30005, LSB_CAST(button2.getcount()));       // for counter 2 (LSB)
...
bool update_holdingReg() {
  ...
  if (slave._device->get(2) != relay2_state ) {  //will add state concept
    ...
    digitalWrite(OUT_RLY_2,relay2_state); 
    _update = true;
  }
  ...
  if (running_var.device_id != slave._device->get(40001)) {
    running_var.device_id =  slave._device->get(40001);      // use for eeprom
    regBank.set(40001,  running_var.device_id);              // update reg
    _update = true;
  }
  ...
  if (running_var.mod_config != slave._device->get(40002)) {
    ...
    regBank.set(40002,  running_var.mod_config);              // update reg
    _update = true;
  }
  ...
  if (running_var.mod_baud != slave._device->get(40003)) {
    ...
    regBank.set(40003,  running_var.mod_baud);              // update reg
    _update = true;
  }
  ...
  if (running_var.net_timeout != slave._device->get(40004)) {
    ...
    regBank.set(40004,  running_var.net_timeout);              // update reg
    _update = true;
  }
  ...
  if (running_var.debounce_tm != slave._device->get(40005)) {
    ...
    regBank.set(40005,  running_var.debounce_tm);              // update reg 
    _update = true;
  }
  ...
  //always add MSB first then LSB 
   ...
   if (slave._device->get(40008) != NORMAL_STATE) { //40008 (MSB) 
    ...
    current_epoch &= 0x0000FFFF;  // Clear the higher 16 bits
    current_epoch |= MSB_SHIFT(slave._device->get(40008));  // Set the higher 16 bits to the new value
  ...
  //set count_1 from modbus protocol
   ...
   if (slave._device->get(40010) != NORMAL_STATE) { //40010 (MSB) 
    ...
    current_count &= 0x0000FFFF;  // Clear the higher 16 bits
    current_count |= MSB_SHIFT(slave._device->get(40010));  // Set the lower 16 bits to the new value 
  ...
  if (slave._device->get(40009) != NORMAL_STATE) { //40009 (LSB) 
   ...
   if (slave._device->get(40012) != NORMAL_STATE) { //40012 (MSB)
    uint32_t current_count  = button2.getcount();
    current_count &= 0x0000FFFF;  // Clear the higher 16 bits
    current_count |= MSB_SHIFT(slave._device->get(40012));  // Set the lower 16 bits to the new value 
  ...
  if (slave._device->get(40013) != NORMAL_STATE) { //400013  
       DEBUG_PRINT("save_command:");  DEBUG_PRINTLN(slave._device->get(40013));
    if(slave._device->get(40013) == SAVE_CONFIG){
       DEBUG_PRINTLN("save running_variable");
       internal_flash.set_struct_var(&running_var,sizeof(running_var)); //set updated parameter to flash
    }
    
    if(slave._device->get(40013) == SAVE_DEFAULT){
       DEBUG_PRINTLN("save default_variable");
       internal_flash.set_struct_var(&default_var,sizeof(default_var)); //set default parameter to flash
    }
    
    regBank.set(40013,  NORMAL_STATE);              // update reg to defaulst
    _update = true;
  }
