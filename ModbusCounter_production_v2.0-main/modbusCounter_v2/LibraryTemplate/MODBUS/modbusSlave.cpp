...
modbusSlave::modbusSlave(HardwareSerial& serialInstance)
{
	serialPort = &serialInstance; 
}
/*
...
void modbusSlave::setBaud(word baud,byte serialconfig)
{
  ...
  //ModSerial.begin(baud,serialconfig);
   ...
   //serialPort->begin(baud,serialconfig);
...
int modbusSlave::checkSerial(void)
{
  ...
  while(serialPort->available()> _len)
  {
    //update the incoming query message length
    _len = serialPort->available();
